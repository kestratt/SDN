...
if (-NOT $NoPrompt.IsPresent)
{
    ...
    Write-Host -ForegroundColor Green "Monitoring Kubelet.err.log for JSON error and will stop tracing"

    Get-Content -Path C:\k\containerd.err.log -Wait | Select-String -Pattern "The requested operation for attach namespace failed.: unknown" -Quiet |% {

        if($_ -eq $True) {Write-Host "The error has been hit hcs::CreateComputeSystem The requested operation for attach namespace failed.: unknown"; break}
